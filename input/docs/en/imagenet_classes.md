title: Display 1000 classes of ImageNet database through convolutional neural network Alexnet
Description: Simple displaying 1000 classification classes from the image database ImageNet for easy searching individually classes
---
```matlab
net = alexnet; % load a network
ClassNames = net.Layers(end).ClassNames; %save all of the class names in to variable
ClassIndexes = string(num2cell(1:1000)'); % create indexes for positions
result = strcat(ClassIndexes,repmat(":",1000, 1),str); % save the names and indexes in to variable and align them horizontally

disp(result) % Display results
```
# Results
